...
pce_versus_unrate <- function(long_pce, graphic_title="This is a test title.", months_highlighted = 6){
  ...
  subtitle <- paste0("Change in year-over-year core PCE from 1 year ago, vs unemployment rate, change < 0, from ", format(min(long_data$date), "%B %Y"), " to ", format(max(long_data$date), "%B %Y"), ".")
  long_data %>%
    filter(YoYD <= 0) %>%
