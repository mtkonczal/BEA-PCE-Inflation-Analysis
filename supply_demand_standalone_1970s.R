...
########### THE BIG ONE
PCE_Weight <- get_NIPA_data(beaKey, 'U20405', 'A', '1972,1973,1974,1975,1976,1977', data_set_name = 'NIUnderlyingDetail')
...
pce <- get_NIPA_data(beaKey, 'U20404', 'A', '1972,1973,1974,1975,1976,1977', data_set_name = 'NIUnderlyingDetail')

PCE_Q <- get_NIPA_data(beaKey, 'U20403', 'A', '1972,1973,1974,1975,1976,1977', data_set_name = 'NIUnderlyingDetail')
...
recent <- pce %>% group_by(TimePeriod) %>%
  ...
  filter(TimePeriod == 1975 | TimePeriod == 1973) %>%
  # Take the difference in rates, so using a deceleration, and keep the latest weight.
  summarize(QuantityFinal = QuantityFinal[TimePeriod == 1975] - QuantityFinal[TimePeriod == 1973],
            PriceFinal = PriceFinal[TimePeriod == 1975] - PriceFinal[TimePeriod == 1973],
            weight = PCEweight[TimePeriod == 1975]) %>%
  ungroup()
